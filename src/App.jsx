...
export default function App() {
    ...
    const handleNativeSelection = (code) => {
        ...
        setShowInitialTargetModal(true); // Open Step 2: Learning Selection
    };
    ...
    const startPractice = () => {
        ...
        if (filteredWords.length === 0) {
            alert(t.noWordsFound || "No words found.");
            return;
        }
    ...
    if (view === "home") {
        return (
            <div className="app-container">
                {/* 1. Welcome Modal (Force Native Language Selection on First Boot) */}
                {showWelcomeModal && (
                ...
                {/* 2. Initial Target Language Modal (What to learn immediately after native) */}
                                ...
                                {availablePracticeLangs.map((lang) => (
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            setShowInitialTargetModal(false);
                        ...
                        <div
                            ...
                            <div className="lang-options-container">
                                {appConfig.languages.map((lang) => (
                                    <button
                                        key={lang.code}
                                        className={`lang-option-btn ${nativeLangCode === lang.code ? "active" : ""}`}
                                        onClick={() => {
    ...
    // Dynamic Language Mapping based on Local Storage Logic
    const targetWord = currentWord[activeLangCode];
    const nativeWord = currentWord[safeNativeLang];
                            ...
                            <div className="card-content">
                                ...
                                {pronunciationText && (
                                    <span className="pronunciation-text">
                                        {t.pronunciation}: {pronunciationText}
                                    </span>
                                )}
